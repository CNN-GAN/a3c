...
class PolicyEstimator():
	...
	"""
	def __init__(self, input_size, num_outputs, trainable=True):
		self.input_size = input_size
		self.num_outputs = num_outputs
		self.use_state_dependent_std = True
		# Placeholders for our input
		# the env feedback observation
		self.states = tf.placeholder(shape=[None, input_size], dtype=tf.float32, name="states")
		# The TD target value
		self.advantages = tf.placeholder(shape=[None], dtype=tf.float32, name="advantage")
		# which action was selected
		self.actions = tf.placeholder(shape=[None, num_outputs], dtype=tf.float32, name="actions")
	...
	def choose_action(self, sess, s):  # run by a local
		...
		action_out = [0,0,0,0,0]
		action_out[0] = action[0][0]
		action_out[1] = action[0][1]
		print (action_out)
		return action_out, params[0]
...
class ValueEstimator():
	...
	def __init__(self, input_size, trainable=True):
		# Placeholders for our input
		self.input_size = input_size
		self.states = tf.placeholder(shape=[None, input_size], dtype=tf.float32, name="states")
		# The TD target value
