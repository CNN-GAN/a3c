...
class Worker(object):
	...
	def __init__(self, name, env, policy_net, value_net, discount_factor, args):
		...
		with tf.variable_scope(name):
			self.policy_net = PolicyEstimator(policy_net.input_size, policy_net.num_outputs)
			self.value_net = ValueEstimator(policy_net.input_size)
	...
	def _setup_shared_memory_ops(self):
		...
		for i in range(len(self.local_policy_params)):
	...
	def _run_master(self, sess):
		for epoch in range(self.num_epochs):
			...
			if epoch == 0:
				...
				ckpt = tf.train.get_checkpoint_state(self.model_dir+'model.ckpt')
			...
			print ('after epoch 0 1')

			#collect worker experience
			episode_rewards = list()
			for i in range(self.episodes_per_batch):
				print ('experience_get', i)
	...
	def _run_worker(self, sess):
		while True:
			...
			while not episode_over and len(accumulated_rewards) < self.max_rollout:
				if self.name == 'W_1':
					self.env.render()
				action = self.policy_net.choose_action(sess, self.state)
				print ('action', action)
